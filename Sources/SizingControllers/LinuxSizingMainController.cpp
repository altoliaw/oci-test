/**
 * @see LinuxSizingMainController.hpp
 */
...
std::map<std::tuple<uint32_t, uint32_t, uint16_t, uint16_t>, char> LinuxSizingMainController::sessionMap;
// For recording the maximum number of packets which contain SQL per second
long LinuxSizingMainController::currentSqlMaxRequestNumberPerSec = 0;
// For reserving the starting time in the beginning or the updating time when the SQL statements receive
std::chrono::steady_clock::time_point LinuxSizingMainController::startingTime = std::chrono::steady_clock::time_point::min();
// For recording the maximum size of tx packets per second
long LinuxSizingMainController::currentMaxTxSizePerSec = 0;
// For recording the maximum size of rx packets per second
long LinuxSizingMainController::currentMaxRxSizePerSec = 0;
// For reserving the starting time in the beginning or the updating time when receiving a tx packet
std::chrono::steady_clock::time_point LinuxSizingMainController::startingTimeTX = std::chrono::steady_clock::time_point::min();
// For reserving the starting time in the beginning or the updating time when receiving a rx packet
std::chrono::steady_clock::time_point LinuxSizingMainController::startingTimeRX = std::chrono::steady_clock::time_point::min();
 ...
 */
Commons::POSIXErrors LinuxSizingMainController::start(int argC, char** argV) {
    ...
    signal(SIGINT, LinuxSizingMainController::signalInterruptedHandler);
 ...
 */
Commons::POSIXErrors LinuxSizingMainController::config(std::vector<unitService>* services) {
 ...
 */
void LinuxSizingMainController::packetTask(PCAP::LinuxPCAP* pcap, void (*packetHandler)(u_char*, const pcap_pkthdr*, const u_char*)) {
 ...
 */
void LinuxSizingMainController::packetFileTask(FILE** fileDescriptor, const char* filePath) {
    // Installing a signal handler, alarm
    signal(SIGALRM, LinuxSizingMainController::signalAlarmHandler);
    ...
    if (*_FILE_POINTER_ == nullptr) {
        *_FILE_POINTER_ = fopen(filePath, "a+");
        if (*_FILE_POINTER_ == nullptr) {
            ...
            LinuxSizingMainController::signalInterruptedHandler(0);  // Going to the end of the thread
 ...
 */
void LinuxSizingMainController::packetHandler(u_char* userData, const struct pcap_pkthdr* pkthdr, const u_char* packet) {
    ...
    if (linuxPCAP != nullptr) {
        ...
        std::pair<std::map<std::tuple<uint32_t, uint32_t, uint16_t, uint16_t>, char>::iterator, bool> insertedResult = SizingControllers::LinuxSizingMainController::sessionMap.emplace(sortedSessionTuple, previousPacketType);
 ...
 * @return [void] None
 */
void LinuxSizingMainController::executePacketInformationUpdate(long long packetHeaderLength,
 ...
 */
void LinuxSizingMainController::signalInterruptedHandler(int) {
    std::cerr << "\n"
 ...
 */
void LinuxSizingMainController::signalAlarmHandler(int) {
    // File writing
    if (*_FILE_POINTER_ == nullptr) {
        // Opening the file
